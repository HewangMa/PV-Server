...
def merge_videos(end_dir):
    file_name = os.listdir(end_dir)[0].split('.')[0]+'.mp4'
    output_file = os.path.join(os.path.dirname(end_dir), file_name)

    videos = sorted([os.path.join(end_dir, f) for f in os.listdir(
        end_dir) if f.lower().endswith(tuple(vid_forms))])
    if not videos:
        print("没有找到视频文件！")
        return
